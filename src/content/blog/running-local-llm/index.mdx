...
For hosting LLMs, the most important part is the Graphics Processing Unit (GPU). This is a module that specialises in parallel computation which essentially means doing a large number of calculations all at once. This is as opposed to the Central Processing Unit (CPU) which specialises in sequential computation which essentially means does a few calculations very quickly. GPUs can be likened to large bus 🚌 while CPUs can be liken to single-seater race cars 🏎️. The race car will get to the destination quicker but the bus would transport more people at the same time.
...
The LLM succeeded in naming the event. That is impressive. However, it was less successful in detecting the number of people. It said 13 but there are 14 people there. The names were completely wrong for most people in the picture unfortunately.
...
|On the first attempt, the answer is wrong. After 2 more tries, ```gemma3:12b``` found the right answer. |On the first attempt, the answer is wrong. ```llama3.1:8b ``` got it right after another try but got it wrong for the next 3 tries after it.|
...
1) Local LLMs are very viable for tasks such as suggesting recipe ideas, formatting text into the MD format (as I did in this article), simple and narrowly defined coding tasks such as adding to the CSS files in this website. 
2) There are limitations to local LLMs. The accuracy can be quite suspect compared to the latest cloud models, especially for logic and images. The fact that many cloud LLMs are connected to the web for realtime information also makes a difference.
...
4) Local LLMs fit neatly into my homelab trajectory now that I have it running on my LAN and can access it though any connected device. I plan on setting up a local VPN so that I can access this externally as well,

In the future, I would like to explore LLMs further for coding and as AI agents. I would also like to try Ollama Cloud's web search for realtime information. Although this will not be self-hosted, it is a good compromise.
